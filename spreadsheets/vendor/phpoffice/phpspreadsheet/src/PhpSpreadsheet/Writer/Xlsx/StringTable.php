    ...
    public function createStringTable(Worksheet $pSheet, $pExistingTable = null)
    {
        ...
        foreach ($pSheet->getCoordinates() as $coordinate) {
            ...
            if (
                ...
                ($cellValue !== null) &&
                !isset($aFlippedStringTable[(string)$cellValue->getHashCode()])
            ) {
                $aStringTable[] = $cellValue;
                $aFlippedStringTable[(string)$cellValue->getHashCode()] = true;
            }
        }
    ...
    public function flipStringTable(array $stringTable)
    {
        ...
        foreach ($stringTable as $key => $value) {
            if (!$value instanceof RichText) {
                $returnValue[(string)$value] = $key;
            } elseif ($value instanceof RichText) {
                $returnValue[(string)$value->getHashCode()] = $key;
            }
        }
