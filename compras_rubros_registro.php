<?
	...
	if($_POST['guardar_rubro'])
	{
		$fdig=date(YmdHis);
		if($_POST['n_codigo'])
		{
			$result=$Db->update('mp_maes_comp_rubro',['x_nombre'=>$_POST['x_nombre'],
			'n_estado'=>$_POST['n_estado'] ]  , ['n_codigo'=>$_POST['n_codigo']]  );
		}
		else
		{
			$result=$Db->insert('mp_maes_comp_rubro',['x_nombre'=>$_POST['x_nombre'],
		...
		echo"
			<!--<script>alert('".CONST_MENS_REG_OK."');</script>-->
                        <html><body>
                                <form name=\"form\" method=post action=\"compras_rubros.php\">
	...
	$result_documento=$Db->select('mp_maes_comp_rubro', ['n_codigo'=>$_POST['n_codigo']], '', '', '');
...
<html>
	<head>
		<meta charset="utf-8" />
		<title>RUBROS / ACTIVIDADDES COMERCIALES</title>
		...
		<script>
			function f_guardar()
			{
					...
					{
						alert('Ingrese el la actividad comercial');
					...
					{
						document.form.guardar_rubro.value='1';
						document.form.submit();
					}
					else
						return false;
			...
			function f_cancelar()
			{
				document.form.action='compras_rubros.php';
				document.form.submit();
			}
...
<?
	if($_POST['n_codigo'])
		...
		echo"Editar Informaci&oacute;n - Actividad Comercial : <i>" .$_POST['x_nombre'] . "</i>";
	else
		echo"REGISTRAR NUEVOS RUBROS / ACTIVIDADES COMERCIALES";
?>
	</h2></center>
		<form name="form" method="post" ENCTYPE="multipart/form-data">
			<input type=hidden name="guardar_rubro">
...
<?
	...
	echo $html->put_text('text',"Rubro&nbsp;/&nbsp;Actividad&nbsp;comercial","Descripci&oacute;n de la actividad",'x_nombre',$_POST['x_nombre'],'','50','');
