...
//classes/TCPDF/examples/personal_fotocheck.php
	...
	if($_POST['actu_depe'])
	{
        ...
        foreach($result as $rows)	
		{
		    ...
		    {
		       // echo"<HR>$vari";
			    //$result=$Db->insert('mp_mpar_despachos',['codi_depe'=>$rows['codi_depe']]);
				...
				foreach($result1 as $rows1)
				{
				    ...
				    //echo"<HR>"."insert into mp_mpar_despachos values('".$rows['codi_depe']."','".$rows1['iden_pers']."','".$arra_flag[$rows['codi_depe']][$rows1['iden_pers']]."')";
				}
		    }
		}
    ...
    $result=$Db->query("select * from mp_mpar_despachos");
    foreach($result as $rows)
...
<html>
	<head>
		...
		<script>
			...
			function f_nuevo()
			{
				...
				document.form.action='mpartes_mantenimiento_despachos.php';
				document.form.target="";
				document.form.submit();
			}
	...
	<center><h2 style="color:#073A6B">AGREGAR DESPACHOS FISCALES</h2></center>
