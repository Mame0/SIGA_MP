...
<?
	...
	foreach($result_pagi as $rows)
	{
		...
		$data=[	'1'=>$cont,
			...
			'4'=>(strlen($rows['desd_expe'])==8 ? substr($rows['desd_expe'],6,2).'/'.substr($rows['desd_expe'],4,2).'/'.substr($rows['desd_expe'],0,4) : $rows['desd_expe']) . 
            ' - ' . 
            (strlen($rows['hast_expe'])==8 ? substr($rows['hast_expe'],6,2).'/'.substr($rows['hast_expe'],4,2).'/'.substr($rows['hast_expe'],0,4) : $rows['hast_expe']),
