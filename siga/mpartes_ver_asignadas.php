...
//classes/TCPDF/examples/personal_fotocheck.php
	...
	if($_POST['codi_elim'])
	{
	    $obse="Usuario: ".$_SESSION['logi_oper']."\nFecha: ".date("d/m/Y H:i:s")."\nObservaci贸n: ".$_POST['obse_elim'];
	    $result=$Db->query("update mp_mpar_carpetas set codi_depe='',codi_pers='',fech_asig='',digi_asig='',obse_mpar='$obse' where codi_mpar='".$_POST['codi_elim']."' AND esta_mpar='1'");
	}
...
<html>
	<head>
		...
		<script>
			...
			function f_eliminar(codi,nume,desp,fisc)
			{
			    if(confirm('Seguro que desea eliminar carpeta asignada?\nDOCUMENTO: '+nume+'\nDESPACHO: '+desp+'\nFISCAL:'+fisc))
			    {
	...
	<center><h2 style="color:#073A6B">REPORTE DE ASIGNACIONES<BR><? echo "$nomb_depe<BR>(desde el ".substr($_POST['fech_desd'],8,2).'/'.substr($_POST['fech_desd'],5,2).'/'.substr($_POST['fech_desd'],0,4)." hasta el ".substr($_POST['fech_hast'],8,2).'/'.substr($_POST['fech_hast'],5,2).'/'.substr($_POST['fech_hast'],0,4).")";?></h2></center>
...
<?
    ...
    foreach($result as $rows)
    {
        ...
        if($posi==0)    $posi=100;
            $arra_nomb_fisc[$rows['iden_pers']]=substr($rows['nomb_pers'],0,$posi)."&nbsp;".$rows['appa_pers'];
    }
...
//echo"<HR>select * from mp_mpar_carpetas where esta_mpar=1 AND codi_depe<>0 AND codi_pers<>0 AND depe_mpar='".$_SESSION['codi_depe']."' AND fech_asig>='".str_replace("-","",$_POST['fech_desd']).'000000'."' AND fech_asig<='".str_replace("-","",$_POST['fech_hast']).'999999'."' order by fdig_mpar desc<HR>";	
	...
	$result_pagi=$Db->query("select * from mp_mpar_carpetas where esta_mpar=1 AND codi_depe<>0 AND codi_pers<>0 AND depe_mpar='".$_SESSION['codi_depe']."' AND fech_asig>='".str_replace("-","",$_POST['fech_desd']).'000000'."' AND fech_asig<='".str_replace("-","",$_POST['fech_hast']).'999999'."' order by fdig_mpar limit $busc_limi_pagi,$busc_item_pagi");
	...
	if($busc_tota_item)
	{
		...
		foreach($result_pagi as $rows)
		{
			...
			$data=[	'1'=>$cont,
				...
				'8'=>"<a href=\"javascript:f_eliminar('$rows[codi_mpar]','$rows[mpar_cbar]','".$arra_nomb_depe[$rows['codi_depe']]."','".$arra_nomb_fisc[$rows['codi_pers']]."')\"><img src=\"img/delete.png\" width=\"20\">",
			];
