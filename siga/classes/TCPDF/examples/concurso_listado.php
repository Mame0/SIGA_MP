...
$Db = new Db();
	...
	$subtitulo="EXAMEN: ".$arra_exam[$_POST['codi_exam']]." PROCESO: ".$arra_regi[$arra_proc_regi[$_POST['codi_proc']]]." ".$arra_proc[$_POST['codi_proc']];
...
function poner_cabecera($pdf,$subtitulo,$subsubtitulo,$pagi_actu=1,$pagi_tota=1)
{
	...
	$pdf->SetXY(20,33.5);
	$pdf->Cell(170, 4, "REPORTE DE POSTULANTES", 0, 1, 'C', 0, '', 0, false, 'T', 'M');
	$pdf->SetFont('arialn', '', 11);
	$pdf->SetXY(20,40);
	...
	foreach($result as $rows)
	{
    	...
    	$pdf->MultiCell(18, $alto,$rows['docu_post'], 'B', 'C', 0, 0, '', '', true, 0, false, false, $alto, 'M');
    	$pdf->MultiCell(30, $alto,$rows['appa_post'], 'B', 'C', 0, 0, '', '', true, 0, false, false, $alto, 'M');
    	$pdf->MultiCell(30, $alto,$rows['apma_post'], 'B', 'C', 0, 0, '', '', true, 0, false, false, $alto, 'M');
    	$pdf->MultiCell(50, $alto,$rows['nomb_post'], 'B', 'C', 0, 0, '', '', true, 0, false, false, $alto, 'M');
    	$pdf->MultiCell(18, $alto,$arra_plaz[$rows['codi_plaz']], 'B', 'C', 0, 0, '', '', true, 0, false, false, $alto, 'M');
    	$pdf->MultiCell(17, $alto,$asis, 'B', 'C', 0, 1, '', '', true, 0, false, false, $alto, 'M');
