<?php
// Se inyectan las librer��as CSS y JS necesarias para los componentes del formulario.
echo '
...
class htmlclass
{
	...
	//var $tag;
	function put_select_estado($label, $name, $value, $enable, $disable, $readonly='')//modificado con readonly
    {
    $arra_sele[0]='';
    $arra_sele[1]='';
    ...
    return "
        <div class=\"row\">
            ...
            <div class=\"col-75\">
                <select id=\"$name\" name=\"$name\" $readonly>
                    <option value=\"1\" {$arra_sele[1]}>$enable</option>
                    <option value=\"0\" {$arra_sele[0]}>$disable</option>
                </select>
            </div>
        </div>
    ";
	...
	public function put_select($label,$name,$arra_options,$value,$others,$id_div='',$only_tag='')
	{
		...
		if (is_array($arra_options)) {
			foreach($arra_options as $cod => $nam)
			{
				...
				{
					$sele=$arra_sele[$cod] ?? null;
					$options.="<option value=\"$cod\" $sele>$nam</option>";
				}
			}
		}
		if($label)
...
<input type=\"$tipo\" id=\"$nomb2\" name=\"$nomb2\" disabled value=\"$value\" placeholder=\"$place\" minlength=\"$min_size\" maxlength=\"$max_size\" $others>
	...
	function put_upload_file($label,$name,$file,$others)
	{
		...
		return "
			<div class=\"row\">
				...
				<input type=\"button\" class=\"button_foot\" value=\"$title\">
			</div>
		";
	}
	...
	function put_text($type,$label,$place,$name,$value,$min_size,$max_size,$others)
	{
        $la = '';
		if($label)
			$la = "<div class=\"col-25\"><label for=\"$name\" style=\"min-width:180px;display:inline-block;white-space:nowrap;overflow:visible;\">$label</label></div>";
	...
	function put_textarea($label,$name,$value,$others)
	{
		...
		return "
			<div class=\"row\">
				$la
				<div class=\"col-75\">
					<textarea id=\"$name\" name=\"$name\" $others rows=\"4\" cols=\"50\">".htmlspecialchars((string)$value, ENT_QUOTES)."</textarea>
				</div>
			</div>
		";
	}
	...
	function put_submit($title,$function,$others="")
	{
		return "
			<div class=\"row\" style=\"margin-top:20px;text-align:center;\">
				<input type=\"submit\" class=\"button_foot\" value=\"$title\" onclick=\"$function\" $others>
			</div>
		";
	}
	...
	function put_window2($cont, $anch, $alto, $titu = "SIOJ Alimentos")
	{
		return"hola";
	}
	function put_window($id, $button_id, $cont, $titu = "SIOJ Alimentos", $anch = 200, $alto = 200)
	{
		return "
			<style>
				.modal
...
<button class=\"button_foot\" onclick=\"return f_ejecutar_$id()\">Aceptar &raquo;</button>
	...
	function put_window_pdf($file, $titu = "SIOJ Alimentos")
	{
		return"
			<style>
				.modal
