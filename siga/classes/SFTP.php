...
$this->cd("ftp");
	...
	public $raiz="ftp/";
	public function SFTP_Rename()
	{
		if($this->connect())
		{
			print "Connection successful";
			$this->rename("sello4.png", "sello5.png");
		}
		else
			print "Connection NO successful";
	}
	public function SFTP_get_name($denu,$ddoc,$Db)
	{
		$result=$Db->select(['ali_denu_demanda',['fdig_denu']],['codi_denu'=>"$denu"]);
		$anno=substr($result[0]['fdig_denu'],0,4);
		$mess=substr($result[0]['fdig_denu'],4,2);
		$this->SFTP_crea_directorios($anno,$mess);
		$result=$Db->select(['ali_denu_documento',['codi_tdoc']],['codi_ddoc'=>"$ddoc"]);
		$tdoc=str_pad($result[0]['codi_tdoc'],3,'0',STR_PAD_LEFT);
		$denu=str_pad($denu,9,'0',STR_PAD_LEFT);
		$ddoc=str_pad($ddoc,9,'0',STR_PAD_LEFT);
		$name="$anno/$mess/docu_".$denu."_".$ddoc."_".$tdoc.".pdf";
		return $name;
	}
	public function SFTP_crea_directorios($anno,$mess)
	{
		if (!file_exists($this->raiz."$anno"))
			mkdir($this->raiz."$anno");
		if (!file_exists($this->raiz."$anno"."/"."$mess"))
			mkdir($this->raiz."$anno"."/"."$mess");
	}
	public function SFTP_verifica_archivo($denu,$ddoc,$Db)
	{
		$name=$this->SFTP_get_name($denu,$ddoc,$Db);
		if (file_exists($this->raiz."$name"))
			return true;
		else
			return false;
	}
	public function SFTP_put($orig,$dest)
	{
		if(copy($orig,$this->raiz.$dest))
			return true;
		else
			return false;
	}
