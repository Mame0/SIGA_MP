<?
	...
	if($_POST['guardar_rubro'])
	{
		...
		if($_POST['n_codigo'])
		{
			$result=$Db->update('mp_comp_lugarentrega',['x_nombre'=>$_POST['x_nombre'],
			'n_estado'=>$_POST['n_estado'] ]  , ['n_codigo'=>$_POST['n_codigo']]  );
		}
		else
		{
			$result=$Db->insert('mp_comp_lugarentrega',['x_nombre'=>$_POST['x_nombre'],
		...
		echo"
			<!--<script>alert('".CONST_MENS_REG_OK."');</script>-->
                        <html><body>
                                <form name=\"form\" method=post action=\"lugar_entrega.php\">
	...
	$result_documento=$Db->select('mp_comp_lugarentrega', ['n_codigo'=>$_POST['n_codigo']], '', '', '');
...
<html>
	<head>
		<meta charset="utf-8" />
		<title>LUGARES DE ENTREGA</title>
		...
		<script>
			function f_guardar()
			{
					...
					{
						alert('Ingrese lugar de entrega');
			...
			function f_cancelar()
			{
				document.form.action='lugar_entrega.php';
				document.form.submit();
			}
...
<?
	if($_POST['n_codigo'])
		...
		echo"Editar Informaci&oacute;n : <i>" .$_POST['x_nombre'] . "</i>";
	else
		echo"REGISTRAR NUEVOS LUGARES DE ENTREGA";
?>
	</h2></center>
...
<?
	...
	echo $html->put_text('text',"Lugar&nbsp;de&nbsp;entrega","Descripci&oacute;n",'x_nombre',$_POST['x_nombre'],'','50','');
