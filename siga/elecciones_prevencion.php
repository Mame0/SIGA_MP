...
<html>
	...
	<center><font style="color:#073A6B;font-weight: bold;"><?=$_POST['nomb_elec']?><BR><font style="font-size: 20;">ACTUACIONES PREVENTIVAS</font><br>USUARIO: <?=$_SESSION['logi_oper']?></font></center>
...
<?
	...
	$result=$Db->query("select codi_prev,codi_tpre,fech_prev,ubig_prev from mp_elec_prevencion WHERE codi_elec='".$_POST['codi_elec']."' AND codi_usua='".$_SESSION['logi_oper']."'");
	$busc_tota_item=0;
	foreach($result as $rows)
	{       
		$busc_tota_item++;
	...
	$result_pagi=$Db->query("select codi_prev,codi_tpre,fech_prev,ubig_prev,esta_prev from mp_elec_prevencion WHERE codi_elec='".$_POST['codi_elec']."' AND codi_usua='".$_SESSION['logi_oper']."' order by codi_prev limit $busc_limi_pagi,$busc_item_pagi");
	...
	$head=['1'=>"NRO.",'2'=>"TIPO",'3'=>"FECHA",'4'=>"UBICACION",'5'=>"EDITAR",'6'=>"ACTA"];
	...
	if($busc_tota_item)
	{
		...
		foreach($result_pagi as $rows)
		{
			...
			$data=[	'1'=>$del.$cont,
				...
				'5'=>"<a href=\"javascript:f_editar('$rows[codi_prev]')\"><img src=\"img/icons/edit.svg\" width=\"20\">",
