...
<html>
	<head>
		...
		<script>
			...
			function f_editar(codi)
			{
				...
				document.form.action='plantilla_plazasvacantes_registro.php';
				document.form.target="";
				document.form.submit();
			}
			function f_nuevo()
	...
	<center><h2 style="color:#073A6B">CONFIGURACION DE PLAZAS VACANTES</h2></center>
...
<?
	...
	$result=$Db->query("select * from mp_plan_plazasvacantes ");
	$busc_tota_item=0;
	...
	$result_pagi=$Db->query("select mp_plan_plazasvacantes.*, mp_plan_escalaremunerativa.esccargo
	from mp_plan_plazasvacantes left join mp_plan_escalaremunerativa on mp_plan_plazasvacantes.codcargo=mp_plan_escalaremunerativa.n_codigo
	order by anoplaza desc, mesplaza desc limit $busc_limi_pagi,$busc_item_pagi");

	echo"<div style=\"width:90%;max-width:800px;margin:auto;\">";
	echo $html->put_title_demand("PLAZAS VACANTES CONFIGURADAS: $busc_tota_item ENCONTRADOS");
	if($busc_tota_pagi>0)
		echo $html->put_page("1",$_POST['busc_pagi_actu'],$busc_tota_pagi,"");
	$head=['1'=>"#",'2'=>"MES&nbsp;A&Ntilde;O",'3'=>"META",'4'=>"CARGO",'5'=>"PLAZAS",'6'=>"EDITAR"];
	...
	if($busc_tota_item)
	{
		...
		foreach($result_pagi as $rows) {
			...
			if ($rows['mesplaza']==1) {$elmes="ENERO";}
			if ($rows['mesplaza']==2) {$elmes="FEBRERO";}
			if ($rows['mesplaza']==3) {$elmes="MARZO";}
			if ($rows['mesplaza']==4) {$elmes="ABRIL";}
			if ($rows['mesplaza']==5) {$elmes="MAYO";}
			if ($rows['mesplaza']==6) {$elmes="JUNIO";}
			if ($rows['mesplaza']==7) {$elmes="JULIO";}
			if ($rows['mesplaza']==8) {$elmes="AGOSTO";}
			if ($rows['mesplaza']==9) {$elmes="SEPTIEMBRE";}
			if ($rows['mesplaza']==10) {$elmes="OCTUBRE";}
			if ($rows['mesplaza']==11) {$elmes="NOVIEMBRE";}
			if ($rows['mesplaza']==12) {$elmes="DICIEMBRE";}

			$cont++;
			$data=[	'1'=>$cont,
				'2'=> $elmes . " " .$rows['anoplaza'],
				'3'=>$rows['meta'],
				'4'=>utf8_encode($rows['esccargo']),
				'5'=>$rows['nroplazas'],
				'6'=>"<a href=\"javascript:f_editar('$rows[autogen]')\"><img src=\"img/icons/edit.svg\" width=\"20\">",
			];
			echo $html->put_table_responsive_data($head,$data);
		}
	}
	else
		echo $html->put_table_responsive_title("No existen plazas vacantes configuradas");
