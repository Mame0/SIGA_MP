...
<html>
	...
	<center><font style="color:#073A6B;font-weight: bold;"><?=$_POST['nomb_elec']?><BR><font style="font-size: 20;">ACTIVIDADES DE DIFUSION</font><br>USUARIO: <?=$_SESSION['logi_oper']?></font></center>
...
<?
	...
	$result=$Db->query("select codi_difu,codi_tdif,fech_difu,ubig_difu from mp_elec_difusion WHERE codi_elec='".$_POST['codi_elec']."' AND codi_usua='".$_SESSION['logi_oper']."'");
	$busc_tota_item=0;
	foreach($result as $rows)
	{       
		$busc_tota_item++;
	...
	$result_pagi=$Db->query("select codi_difu,codi_tdif,fech_difu,ubig_difu from mp_elec_difusion WHERE codi_elec='".$_POST['codi_elec']."' AND codi_usua='".$_SESSION['logi_oper']."' order by fdig_difu limit $busc_limi_pagi,$busc_item_pagi");
	...
	$head=['1'=>"NRO.",'2'=>"TIPO",'3'=>"FECHA",'4'=>"UBICACION",'5'=>"EDITAR"];
	...
	if($busc_tota_item)
	{
		...
		foreach($result_pagi as $rows)
		{
			...
			$data=[	'1'=>$cont,
				...
				'5'=>"<a href=\"javascript:f_editar('$rows[codi_difu]')\"><img src=\"img/icons/edit.svg\" width=\"20\">",
			];
