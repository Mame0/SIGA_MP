...
//classes/TCPDF/examples/personal_fotocheck.php
	...
	$pagi=$_SERVER["REQUEST_URI"];
        $fdig=date("YmdHis");
        $result=$Db->query("insert into mp_admi_acce(dire_acce,iden_oper,ndoc_oper,diip_acce,fdig_acce,esta_acce) values('$pagi','$_SESSION[iden_oper]','$_SESSION[ndoc_oper]','$dire_auop','$fdig','1')");
?>
<!DOCTYPE html>
<html>
	<head>
...
<?
	...
	$result=$Db->query("SELECT DISTINCT SUBSTRING_INDEX(SUBSTRING_INDEX(tx_formato, '-', 2), '-', -1) AS anno FROM mp_cons_firm WHERE tx_formato LIKE '%-%' ORDER BY anno ASC");
	foreach($result as $rows)
	{
		$anno = $rows['anno'];
		// Se asegura que el valor extraído sea un año válido de 4 dígitos.
		if (is_numeric($anno) && strlen($anno) == 4 && $anno > 1900 && $anno < 2100) {
			$arra_options_anno[$anno] = $anno;
		}
	}
...
if($_POST['expe_firm'])
{
    ...
    foreach($result_pagi as $rows)
    {
		...
		$data=[	'1'=>$cont,
				...
				'5'=>"<font color=red><b>".substr($rows['f_firma'],0,10),
