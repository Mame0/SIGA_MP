<?
	...
	if($_POST['guardar_personal'])
	{
		...
		else
		{
			...
			$_POST['codi_dete']=$Db->lastInsertId();
		}
		...
		if($_FILES['file_docu']['name'] AND $_FILES['file_docu']['size']>0)
		{
			if(strstr($_FILES['file_docu']['type'],"pdf") OR strstr($_FILES['file_docu']['type'],"pdf"))
			{
				...
				move_uploaded_file($_FILES['file_docu']['tmp_name'],"actas/dete_".str_pad($_POST['codi_dete'], 6, "0", STR_PAD_LEFT).".pdf");
			}
			else
...
<html>
	<head>
		...
		<script>
			...
			function f_cancelar()
			{
				document.form.action='elecciones_detenciones.php';
				document.form.submit();
			}
	...
	<center><font style="color:#073A6B;font-weight: bold;"><?=$_POST['nomb_elec']?><BR><font style="font-size: 20;">REGISTRAR NUEVA DETENCION E INTERVENCION</font><br>USUARIO: <?=$_SESSION['logi_oper']?></font></center>
...
<?
	...
	echo $html->put_select("Detenido&nbsp;o&nbsp;Intervenido",'dete_inte',$arra_options_dete_inte,$_POST['dete_inte'],"");
	echo $html->put_text('date',"Fecha","Ingrese Fecha ",'fech_dete',$_POST['fech_dete'],'','8','');
	echo $html->put_text('time',"Hora","Ingrese Hora ",'hora_dete',$_POST['hora_dete'],'','8','');
	echo"</main><main>";
	echo $html->put_select("Provincia&nbsp;-&nbsp;Distrito",'ubig_dete',$arra_options_prov,$_POST['ubig_dete'],"");
	echo $html->put_text('text',"Lugar","Ingrese Lugar de intervenci&oacute;n o detenci&oacute;n",'luga_inte',$_POST['luga_inte'],'','100','');
	echo $html->put_select("Accionar&nbsp;del&nbsp;MP",'codi_acci',$arra_options_acci,$_POST['codi_acci'],"");
	
	echo $html->put_title_demand("Motivo de Intervenci&oacute;n o Detenci&oacute;n");
	echo $html->put_select("Motivo",'codi_inte',$arra_options_inte,$_POST['codi_inte'],"");
	echo $html->put_text('time',"Hora","Ingrese Hora ",'hora_moti',$_POST['hora_moti'],'','8','');
	echo $html->put_text('text',"Detalle","Ingrese Detalle",'deta_moti',$_POST['deta_moti'],'','100','');
	
	echo $html->put_title_demand("Detenido o Intervenido");
	echo $html->put_text('text',"Nombres&nbsp;y&nbsp;Apellidos","Ingrese Nombres",'nomb_dete',$_POST['nomb_dete'],'','200','');
	echo $html->put_text('text',"DNI","Ingrese DNI",'ndni_dete',$_POST['ndni_dete'],'','8','');
	echo $html->put_text('text',"Edad","Ingrese Edad",'edad_dete',$_POST['edad_dete'],'','3','');
	echo"</main><main>";
	echo $html->put_select("Sexo",'sexo_dete',$arra_options_sexo,$_POST['sexo_dete'],"");
	echo $html->put_select("Tipo&nbsp;de&nbsp;Delito",'codi_deli',$arra_options_deli,$_POST['codi_deli'],"");
	echo $html->put_textarea("Detalle&nbsp;de&nbsp;los&nbsp;Hechos",'deta_inte',$_POST['deta_inte'],'style="height: 100px;"');
